Feature: Forget/remove User from one client id
  As an admin
  I want to remove a user from a client id
...
When I open postman, assuring the environment is MRI AIG Int - Qa at the top right
...
And then I go to Integration API -> App Users -> Del Try Forget - by username
And at the address bar put the account used to make the test reports and make sure deleteOrphanedReport is False (aig.user10@mrisoftware.disabled?deletedOrphanedReport=false&tryForget=true)
And click Send
