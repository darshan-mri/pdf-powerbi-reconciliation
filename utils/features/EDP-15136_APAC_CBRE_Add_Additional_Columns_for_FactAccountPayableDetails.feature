Feature: Validate newly added Custom Fields column integration for FactAccountPayablesInvoiceDetails Warehouse tables

  Scenario: Validate Custom Fields data load into Warehouse tables after pipeline execution
     Given the Custom Fields column exists in the Config DB mapping for the FactAccountPayablesInvoiceDetails tables
     And the IsWHLoadEnabled and IsFullRefresh flags are set to 1 in the Config DB for FactAccountPayablesInvoiceDetails table
     ...
     And the PlCMFactMaster pipelines is triggered in Azure Synapse
     ...
     Then the Custom Fields column should be created and populated with data in Warehouse tables for below fields
        ...
        |UserCreated |
        |CDateCreated|
        |FundRequestReference|
        |PaymentAuthorisationDate|
        |PaymentAuthorisedby|
        |PaymentAuthorisationTime|
        |FromDate |
        |ToDate   |
        |InvcRef  |
